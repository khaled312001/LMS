    ...
    public function live_class_store(Request $request, $course_id)
    {
        ...
        try {
            ...
            if ($date_timestamp === false) {
                return redirect(route('admin.course.edit', ['id' => $course_id, 'tab' => 'live-class']))->with('error', get_phrase('Invalid date format.'));
            }
            ...
            if ($request->provider == 'zoom') {
                ...
                if (!is_array($meeting_info_arr)) {
                    return redirect(route('admin.course.edit', ['id' => $course_id, 'tab' => 'live-class']))->with('error', get_phrase('Failed to create Zoom meeting. Invalid response.'));
                }
                ...
                if (!isset($meeting_info_arr['id']) || empty($meeting_info_arr['id'])) {
                    return redirect(route('admin.course.edit', ['id' => $course_id, 'tab' => 'live-class']))->with('error', get_phrase('Failed to create Zoom meeting. Meeting ID is missing.'));
                }
            ...
            return redirect(route('admin.course.edit', ['id' => $course_id, 'tab' => 'live-class']))->with('error', get_phrase('An error occurred: ') . $e->getMessage());
        }
    }
