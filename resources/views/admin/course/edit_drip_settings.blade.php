@php
    ...
    // Set default values if drip_content_settings is null or invalid
    if (!is_array($drip_content_settings) || empty($drip_content_settings)) {
        ...
        <div class="eRadios">
            <div class="form-check">
                <input type="radio" value="0" name="enable_drip_content" class="form-check-input eRadioSuccess"
                    id="drip_off" @if (($course_details->enable_drip_content ?? 0) == 0) checked @endif required>
            ...
            <div class="form-check">
                <input type="radio" value="1" name="enable_drip_content" class="form-check-input eRadioPrimary"
                    id="drip_on" @if (($course_details->enable_drip_content ?? 0) == 1) checked @endif required>
...
<div class="row mb-3">
	...
	<div class="col-sm-9">
		<div class="eRadios">
			<div class="form-check">
				<input type="radio" onchange="$('.toggleMinimumWatchField').toggleClass('d-hidden');" value="percentage" id="video_percentage_wise" name="lesson_completion_role" class="form-check-input eRadioSuccess" {{ ($drip_content_settings['lesson_completion_role'] ?? 'percentage') == 'percentage' ? 'checked' : '' }}>
				<label for="video_percentage_wise" class="form-check-label">{{get_phrase('Video percentage wise')}}</label>
			</div>

			<div class="form-check">
				<input type="radio" onchange="$('.toggleMinimumWatchField').toggleClass('d-hidden');" id="video_duration_wise" name="lesson_completion_role" value="duration" class="form-check-input eRadioSuccess" {{ ($drip_content_settings['lesson_completion_role'] ?? 'percentage') == 'duration' ? 'checked' : '' }}>
...
<div class="paid-section toggleMinimumWatchField {{ ($drip_content_settings['lesson_completion_role'] ?? 'percentage') != 'percentage' ? 'd-hidden' : '' }}">
	<div class="row mb-3">
		...
		<div class="col-sm-9">
			<input type="text" value="{{ $drip_content_settings['minimum_percentage'] ?? '30' }}" class="form-control ol-form-control" id="minimum_percentage" name="minimum_percentage" placeholder="50">
		</div>
	</div>
</div>

<div class="paid-section toggleMinimumWatchField {{ ($drip_content_settings['lesson_completion_role'] ?? 'percentage') != 'duration' ? 'd-hidden' : '' }}">
	<div class="row mb-3">
		...
		<div class="col-sm-9">
			<input type="text" value="{{ seconds_to_time_format($drip_content_settings['minimum_duration'] ?? 15) }}" class="form-control ol-form-control" id="minimum_duration" name="minimum_duration" placeholder="00:01:10">
		</div>
	</div>
</div>
