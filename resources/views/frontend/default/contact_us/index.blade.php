            ...
            <div class="row pt-100 align-items-stretch">
                ...
                <div class="col-lg-7">
                    <div class="form-container-vibrant">
                        <h2 class="display-5 g-title mb-4">{{ get_phrase('Send Message') }}</h2>
                        <p class="text-muted mb-5 fs-5">{{ get_phrase('Have a specific inquiry? Fill out the form below and we will get back to you within 24 hours.') }}</p>
                        
                        <form action="{{ route('contact.store') }}" method="post" class="mt-4" id="global-form">@csrf
                            <div class="row">
                                <div class="col-md-6 mb-4">
                                    <div class="form-group">
                                        <label for="name" class="form-label fw-bold text-dark ms-2 mb-2">{{ get_phrase('Name') }}</label>
                                        <input type="text" name="name" class="form-control form-control-vibrant @error('name') border-danger @enderror" placeholder="{{ get_phrase('Your Name') }}">
                                    </div>
                                </div>
                                <div class="col-md-6 mb-4">
                                    <div class="form-group">
                                        <label for="email" class="form-label fw-bold text-dark ms-2 mb-2">{{ get_phrase('Email') }}</label>
                                        <input type="email" name="email" class="form-control form-control-vibrant @error('email') border-danger @enderror" placeholder="{{ get_phrase('Your Email') }}">
                                    </div>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6 mb-4">
                                    <div class="form-group">
                                        <label for="phone" class="form-label fw-bold text-dark ms-2 mb-2">{{ get_phrase('Phone') }}</label>
                                        <input type="tel" name="phone" class="form-control form-control-vibrant @error('phone') border-danger @enderror" placeholder="{{ get_phrase('Your phone') }}">
                                    </div>
                                </div>
                                <div class="col-md-6 mb-4">
                                    <div class="form-group">
                                        <label for="address" class="form-label fw-bold text-dark ms-2 mb-2">{{ get_phrase('Address') }}</label>
                                        <input type="text" name="address" class="form-control form-control-vibrant @error('address') border-danger @enderror" placeholder="{{ get_phrase('Your address') }}">
                                    </div>
                                </div>
                            </div>
                            <div class="mb-4">
                                <div class="form-group">
                                    <label for="message" class="form-label fw-bold text-dark ms-2 mb-2">{{ get_phrase('Message') }}</label>
                                    <textarea name="message" cols="30" rows="6" class="form-control form-control-vibrant @error('message') border-danger @enderror" placeholder="{{ get_phrase('Your message here ...') }}"></textarea>
                                </div>
                            </div>
                            ...
                            @if(get_frontend_settings('recaptcha_status'))
                                <button class="btn btn-vibrant w-100 py-3 rounded-pill fw-bold g-recaptcha" data-sitekey="{{ get_frontend_settings('recaptcha_sitekey') }}" data-callback='onContactSubmit' data-action='submit'>
                                    {{ get_phrase('Send Message') }} <i class="fa-solid fa-paper-plane ms-2"></i>
                                </button>
                            @else
